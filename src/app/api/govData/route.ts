...
export async function GET(request: Request) {
  try {
    ...
    const departmentKey = searchParams.get("departmentKey") || "1421000";
    ...
    if (startDate) {
      ...
      const latest = await db.businessAnnouncement.findFirst({
    ...
    while (currentDate <= endDate) {
      ...
      try {
        ...
        const results = await Promise.all(
          items.map((item) =>
            upsertAnnouncement(item, currentDate, departmentKey),
          ),
        );
